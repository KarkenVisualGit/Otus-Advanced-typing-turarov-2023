...
describe("TaskCalendar init method", () => {
	...
	it("should assign click event listeners to buttons", () => {
		const addOrUpdateTaskButton = document.getElementById(
			"addOrUpdateTaskButton"
		) as HTMLButtonElement;
		const applyFiltersButton = document.getElementById(
			"applyFiltersButton"
		) as HTMLButtonElement;
		const loadFromFirebaseButton = document.getElementById(
			"loadFromFirebaseButton"
		) as HTMLButtonElement;
		const loadFromLocalButton = document.getElementById(
			"loadFromLocalButton"
		) as HTMLButtonElement;
