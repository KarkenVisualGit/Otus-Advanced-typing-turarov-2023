...
class TestableTaskCalendar extends TaskCalendar {
    public async testAddOrUpdateTask(taskId?: string): Promise<void> {
        if (taskId) {
            (document.getElementById('addOrUpdateTaskButton') as HTMLButtonElement).dataset.editingId = taskId;
        }
...
describe('addOrUpdateTask', () => {
    ...
    it('should add a new task to Firebase', async () => {
        ...
        (document.getElementById('saveOption') as HTMLSelectElement).value = 'firebase';
        ...
        expect(set).toHaveBeenCalledWith(`${"mockedDatabaseInstance"}-todos/${mockTask.id}`, {
            text: 'Test Task',
            date: '2023-01-01',
            status: 'new',
            tags: 'tag1, tag2',
    ...
    it('should update an existing task', async () => {
        ...
        (document.getElementById('addOrUpdateTaskButton') as HTMLButtonElement).dataset.editingId = '1';
